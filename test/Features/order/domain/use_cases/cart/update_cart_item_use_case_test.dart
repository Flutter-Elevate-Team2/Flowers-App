...
void main() {
  ...
  setUp(() {
    ...
    updateCartItemUseCase = UpdateCartItemUseCase(mockOrderRepoContract);
  });

  test(
    "should call UpdateCartItemUseCase.call() and return SuccessResponse",
        () async {
      // Arrange
      final quantityRequest = QuantityRequest(quantity: 6);

      when(
        mockOrderRepoContract.updateCartItem(any, any),
      ).thenAnswer(
      ...
      verify(
        mockOrderRepoContract.updateCartItem(
          "123",
          argThat(
            isA<QuantityRequest>()
                .having((q) => q.quantity, 'quantity', 6),
          ),
        ),
      ).called(1);
    },
