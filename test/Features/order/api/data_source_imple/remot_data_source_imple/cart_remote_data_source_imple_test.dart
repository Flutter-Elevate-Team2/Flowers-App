  ...
  group("CartDataSourceImple Tests", () {
    test(
      ...
      () async {
        // Arrange
        when(
          mockCartApi.updateCartProduct(any, any),
        ).thenAnswer((_) async => tCartResponseModel);

        // Act
        final result = await cartRemoteDataSourceImple.updateCartItem(
          "123",
    ...
    test(
      ...
      () async {
        // Arrange
        when(
          mockCartApi.deleteProductFromCart(any),
        ).thenAnswer((_) async => tCartResponseModel);

        // Act
        final result = await cartRemoteDataSourceImple.deleteItemFromCart(
          "123",
        );
    ...
    test(
      ...
      () async {
        // Arrange
        when(
          mockCartApi.addProductToCart(any),
        ).thenAnswer((_) async => tCartResponseModel);
