  ...
  group("CartRepoImple Tests", () {
    test(
      ...
      () async {
        ...
        when(
          mockCartRemoteDataSourceContract.updateCartItem(any, any),
        ).thenAnswer((_) async => tCartResponseModel);

        // Act
        final result = await cartRepoImple.updateCartItem(
          "123",
          quantityRequest,
        );
        ...
        verify(
          mockCartRemoteDataSourceContract.updateCartItem(
            "123",
            argThat(
    ...
    test(
      ...
      () async {
        when(
          mockCartRemoteDataSourceContract.getCartData(),
        ).thenAnswer((_) async => tCartResponseModel);
    ...
    test(
      ...
      () async {
        when(
          mockCartRemoteDataSourceContract.deleteItemFromCart("123"),
        ).thenAnswer((_) async => tCartResponseModel);
    ...
    test(
      ...
      () async {
        when(
          mockCartRemoteDataSourceContract.addToCart(tCartRequest),
        ).thenAnswer((_) async => tCartResponseModel);
